...
$(function(){
	...
	errorinfo_wp.next().html("*");
	
	if(errorinfo.val()==null ||errorinfo.val()==""){
		a_idPicPath.next().html("*上传大小不能超过500k *上传类型必须为：jpg、jpeg、png、pneg");
	}else {
		a_idPicPath.next().html(errorinfo.val());
	}
	if(errorinfo_wp.val()==null ||errorinfo_wp.val()==""){
		a_workPicPath.next().html("*上传大小不能超过500k *上传类型必须为：jpg、jpeg、png、pneg");
	}else {
